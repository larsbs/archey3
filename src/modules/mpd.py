...
@module_register('mpd')
class MpdModule(Display):
    """
    Display certain stat about MPD database. If mpd not installed,
    output nothing.
    """
    ...
    def format_output(self, instring):
        ...
        # If people don't have mpc installed then return None)
            ...
            self.state.logger.error('Could not parse mpc output, is mpc installed?')
            return
